...
const App: FC<AppProps> = ({ scripts = [], dark = false, onSave, onEmitCode }: AppProps) => {
  const [listScrollTop, setListScrollTop] = useState(0);
  const [currentTheme] = useState<StyleTheme>(dark ? StyleTheme.DARK : StyleTheme.LIGHT);
  ...
  const getScriptData = useCallback(
    (scriptIndex?: number): IScriptItem | undefined =>
      scriptIndex === undefined ? undefined : scriptList[scriptIndex],
    [scriptList],
  );

  const handleEmitCode = useCallback(
    (scriptIndex: number) => {
